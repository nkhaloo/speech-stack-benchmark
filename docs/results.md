...
> **Note (provenance, 2026-08-03):** Between 2026-07-21 and 2026-08-03 this file
> reported the *edge-trim-only* numbers (macro DER 0.2459, macro cpWER 0.3487,
> Arabic cpWER 0.4543) — an earlier revision that commit `993eae3` restored by
> mistake while adding the two summary documents. Those numbers are **superseded**;
> anything quoting them, including the emailed PDF of 2026-07-21, should be
> retracted. The figures below were re-verified on 2026-08-03 directly against the
> run cache on the lab machine (`metrics/per_model/diar_by_model.csv`,
> `metrics/per_stack/pair_by_stack.csv`, `metrics/per_language/pair_by_language.csv`),
> and the reference build was confirmed to use voiced-segment turns (2499 of 4790
> reference turns are adjacent same-speaker, the signature of intra-clip splitting).
>
> **Caution on the run id.** All three reference-construction variants were written
> under the single run id `2026-07-19_gpu_baseline_v1`, because the run id keys
> resumability and was not bumped when the reference builder changed. Only the last
> variant survives in the run cache. Any future change to reference construction
> **must** take a new run id.
